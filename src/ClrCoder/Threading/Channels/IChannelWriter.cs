...
namespace ClrCoder.Threading.Channels
{
    ...
    public interface IChannelWriter<T>: IDisposable
    {
        ...
        /// </returns>
        ValueTask<bool> WaitToWriteValueTaskAsync(CancellationToken cancellationToken = default);
        ...
        ValueTask<ValueVoid> WriteValueTaskAsync(T item, CancellationToken cancellationToken = default);
...
#region The zero-copy batch mode interface
        ...
        /// <param name="slice">The buffer slice with to write data to.</param>
        /// <returns>
        /// <see langword="true"/> if the buffer slice was allocated; otherwise, <see langword="false"/> if it wasn't
        /// allocated.
        /// </returns>
        bool TryStartWrite(int count, out ChannelWriterBufferSlice<T> slice);
...
#region Copy-based batch mode interface
        ...
        /// <param name="written">The amount of items written.</param>
        /// <returns>true if the item was written; otherwise, false if it wasn't written.</returns>
        bool TryWrite(ReadOnlySpan<T> items, out int written);
        ...
        /// operation.
        /// </param>
        /// <returns>A <see cref="ValueTask{T}"/> that represents the asynchronous write operation and returns the number of items has been written.</returns>
        ValueTask<int> WriteAsync(ReadOnlySpan<T> items, CancellationToken cancellationToken = default);

#endregion

#region Array pass-through abla interface
        ...
        /// </remarks>
        /// <param name="items">The item to write.</param>
        /// <returns>true if the items was fully written; otherwise, false if it wasn't fully written.</returns>
        bool TryWriteWithOwnership(T[] items);
        ...
        /// </param>
        /// <returns>A <see cref="ValueTask{T}"/> that represents the asynchronous write operation.</returns>
        ValueTask<ValueVoid> WriteAsyncWithOwnership(T[] items, CancellationToken cancellationToken = default);
